...
def run_scout(creep, hostile_constructions):
    """
    just for basic scouting purpose.

    :param creep:
    :param hostile_constructions: 적 건물계획
    :return:
    """

    # todo 방 하나 정찰하고 또 다른곳 가서 등등등.... 하는 역할을 수행해야만 한다.

    if creep.memory.assigned_room == creep.room.name and len(hostile_constructions):
        ...
        if not creep.memory.step_target:
            step_target = creep.pos.findClosestByRange(hostile_constructions)

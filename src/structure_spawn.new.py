...
def run_spawn(spawn, all_structures, room_creeps, hostile_creeps, divider, counter, cpu_bucket_emergency
    ...
    if not spawn.spawning and Game.time % counter == divider:
        ...
        if len(rm_flag) > 0:
            ...
            시험가동
            '''
            for remote_flag in rm_flag:
                # print('flag {}'.format(flag))
                # if seeing the room is False - need to be scouted
                if not Game.flags[remote_flag].room:
                    # look for scouts
                    creep_scouts = _.filter(creeps, lambda c: c.memory.role == 'scout'
                                                              and c.memory.assigned_room == Game.flags[
                                                                  remote_flag].pos.roomName)
                    # print('scouts:', len(creep_scouts))
                    if len(creep_scouts) < 1:
                        spawn_res = spawn.createCreep([MOVE], 'Scout-' + remote_flag,
                                                      {'role': 'scout', 'assigned_room': Game.flags[remote_flag].pos.roomName})
                    ...
                    remote_troops = _.filter(creeps, lambda c: c.memory.role == 'soldier'
                                                               and c.memory.assigned_room == Game.flags[
                                                                   remote_flag].pos.roomName
                                                               and (c.spawning or (c.hits > c.hitsMax * .3
                    ...
                    remote_dem = _.filter(creeps, lambda c: c.memory.role == 'demolition'
                                                            and c.memory.assigned_room == Game.flags[
                                                                remote_flag].pos.roomName)
                    # collector - 잔해 수거반.
                    remote_collector = _.filter(creeps, lambda c: c.memory.role == 'g_collector'
                                                                  and c.memory.assigned_room == Game.flags[
                                                                      remote_flag].pos.roomName)

                    hostiles = Game.flags[remote_flag].room.find(FIND_HOSTILE_CREEPS)
                    hostiles = miscellaneous.filter_allies(hostiles)

                    # 원래 더 아래에 있었지만 키퍼방 문제가 있는지라...
                    flag_structures = Game.flags[remote_flag].room.find(FIND_STRUCTURES)
                    ...
                    # 방에 컨트롤러가 없는 경우 가정.
                    flag_room_controller = Game.flags[remote_flag].room.controller
                    flag_room_reserved_by_other = False
                    print('flag_room_controller', bool(flag_room_controller))
                    # 컨트롤러가 있는가?
                    if flag_room_controller:
                        # 있다면...
                        # 주인이 존재하고 내것이 아닌가?
                        print(Game.flags[remote_flag].room.name)

                        occupied = flag_room_controller.reservation
                        ...
                        # 예약이 돼있는가?
                            ...
                            print(flag_room_controller.reservation.username)
                            print(spawn.owner.username)
                            # 내가 예약한것이 아닌가?
                    ...
                    # 위는 우선 여기까지. 다음은 계속 개편한다.
                    # 만약 지역 땅이 타인에게 점령당한 상태인 경우 군인을 뽑는다.
                    # 다만 적이 있을 시 한정임.
                    if flag_room_reserved_by_other:
                        print('flag_room_reserved_by_other', flag_room_reserved_by_other)
                        # 먼져 적을 찾는다.
                        if len(hostiles) > 0:
                            # 렙7 아래면 스폰 안한다.
                            if spawn.room.controller.level < 7:
                                continue
                            elif len(remote_troops) == 0:
                                spawn_res = spawn.createCreep(
                                    ...
                                    , undefined, {'role': 'soldier', 'soldier': 'remote_defender'
                                        , 'assigned_room': Game.flags[remote_flag].room.name})
                                continue

                        # 하여튼 지금은 적이 없는거 같으니 리서버를 뽑는다.
                        if len(remote_reservers) == 0 \
                                and (not Game.flags[remote_flag].room.controller.reservation
                                     or Game.flags[remote_flag].room.controller.reservation.ticksToEnd < 1200):
                            spawning_creep = spawn.createCreep(
                                [MOVE, MOVE, MOVE, MOVE, CLAIM, CLAIM, CLAIM, CLAIM]
                                , undefined,
                                {'role': 'reserver', 'home_room': spawn.room.name,
                                 'assigned_room': Game.flags[remote_flag].pos.roomName
                                    , 'rm_flag': remote_flag})
                            if spawning_creep == ERR_NOT_ENOUGH_RESOURCES:
                                spawning_creep = spawn.createCreep(
                        ...
                        # 리서버를 뽑았으면 이제 방 안에 건물이 있나 확인한다. 컨테이너·도로는 상관없고 그 외 모든게 철거대상.
                        flag_structures_to_destroy = flag_structures.filter(lambda s:
                                                                            s.structureType != STRUCTURE_ROAD
                                                                            or s.structureType != STRUCTURE_CONTAINER)
                        # 철거할 것이 있으면 철거반을 만든다.
                        if flag_structures_to_destroy:
                            if len(remote_dem) == 0:
                                if spawn.room.controller.level < 7:
                                    ...
                                    body = [MOVE, MOVE, MOVE, MOVE, MOVE, MOVE, MOVE, MOVE, MOVE, MOVE,
                                            ...
                                            WORK, WORK,
                                            WORK, WORK]
                                # 'demo_container': 0 이건 컨테이너도 파괴하는가에 대한 작업인데 현재 개조중이니 무력화.
                                ...
                                continue
                            # 철거반 뒤에는 떨어진 자원 주워먹는 수거반이 있어야 함.
                            elif len(remote_dem) > 0 and remote_collector == 0:
                                _body = [MOVE, MOVE, MOVE, MOVE, MOVE, MOVE, MOVE, MOVE, MOVE, MOVE, MOVE, MOVE, MOVE,
                                         MOVE, MOVE, MOVE, MOVE, MOVE, MOVE, MOVE, CARRY, CARRY, CARRY, CARRY, CARRY,
                                         CARRY, CARRY, CARRY, CARRY, CARRY, CARRY, CARRY, CARRY, CARRY, CARRY, CARRY,
                                         CARRY, CARRY, CARRY, CARRY, HEAL]
                                # todo 수거반 스크립트 만들어야함
                                spawn.createCreep(_body, undefined, {'role': 'g_collector'
                                    , 'assigned_room': Game.flags[remote_flag].pos.roomName, 'home_room': spawn.pos.roomName})
                                continue
                    # 컨트롤러를 적이 점령하지 않았을 경우.
                    # todo 컨트롤러 없는 방인 경우를 고려해야 한다. 현재는 단순 멀티라고 간주
                    else:
                    ...
                    # 만일 해당 방이 다른놈들에 의해 장악된 상태면 아래를 돌릴 이유가 없다.
                    if flag_room_reserved_by_other:
                        continue

                    # find creeps with assigned flag's room.
                    ...
                    flag_energy_sources = Game.flags[remote_flag].room.find(FIND_SOURCES)
                    # FIND_SOURCES가 필요없는 이유: 어차피 그걸 보지 않고 건설된 컨테이너 수로 따질거기 때문...?
                    # ^ 이리 썼는데 지금 이해못함 - 180304
                    flag_containers = _.filter(flag_structures,
                                               lambda s: s.structureType == STRUCTURE_CONTAINER)
                    flag_constructions = Game.flags[remote_flag].room.find(FIND_CONSTRUCTION_SITES)
                    ...
                    if len(flag_energy_sources) > len(remote_carriers):
                        ...
                        # 겹친 컨테이너 확인여부. 추후 겹치는걸 늘릴 경우 대비.
                        merged_container = 1

                        # Memory.rooms.source_code == 소스와 가까이 있는지 확인절차.
                        # 1 == 소스가 하나 | 2 == 소스가 둘이고 근접 | 3 == 그 외
                        if not Memory.rooms[Game.flags[remote_flag].pos.roomName]:
                            # 방 없으면 초기화
                            Memory.rooms[Game.flags[remote_flag].pos.roomName] = {}

                        if not Memory.rooms[Game.flags[remote_flag].pos.roomName].source_code:
                            # 없으면 확인한다.
                            if len(flag_energy_sources) == 1:
                                # 소스가 그저 하나일 경우
                                Memory.rooms[Game.flags[remote_flag].pos.roomName].source_code = 1
                            elif len(flag_energy_sources) == 2:
                                if flag_energy_sources[0].pos.inRangeTo(flag_energy_sources[1], 8):
                                    # todo 하나 문제: 진흙투성이면?
                                    sources_gap = flag_energy_sources[0].pos.findPathTo(flag_energy_sources[1]
                                                                                        , {'ignoreCreeps': True})
                                    # 8칸을 넘기면 떨어져 있는거기 때문에 의미가 없다
                                    if len(sources_gap) > 8:
                                        Memory.rooms[Game.flags[remote_flag].pos.roomName].source_code = 3
                                    else:
                                        Memory.rooms[Game.flags[remote_flag].pos.roomName].source_code = 2
                                else:
                                    Memory.rooms[Game.flags[remote_flag].pos.roomName].source_code = 3
                            else:
                                # 나중에 소스가 셋일 경우 수정해야 하지만 현 바닐라판은 이대로 해도됨.
                                Memory.rooms[Game.flags[remote_flag].pos.roomName].source_code = 3

                        # 소스가 근접할 경우
                        if Memory.rooms[Game.flags[remote_flag].pos.roomName].source_code == 2:
                            # 주변에 컨테이너가 있는가?
                            containers_around = False

                            merged_container = 2

                            for c in flag_containers:
                                if c.pos.inRangeTo(flag_energy_sources[0], 4):
                                    containers_around = True
                                    # 캐리어의 픽업 메모리로 배정.
                                    carrier_pickup = c.id
                                    break
                            # 위에 컨테이너 확인했는데도 건설된게 없으면 위치를 찍는다.
                            if not containers_around:
                                # 소스사이간 거리가 정립 안된 상태면 계산한다.
                                if not sources_gap:
                                    sources_gap = flag_energy_sources[0].pos.findPathTo(flag_energy_sources[1]
                                                                                        , {'ignoreCreeps': True})

                                middle = round(int(sources_gap) / 2)
                                # 컨테이너 건설할 중간지점 위치
                                const_loc = __new__(RoomPosition(sources_gap[middle].x, sources_gap[middle].y
                                                                 , Game.flags[remote_flag].room.name))
                                const_loc.createConstructionSite(STRUCTURE_CONTAINER)
                                ...
                                constr_roads_pos = \
                                    PathFinder.search(constr_pos, spawn.pos
                                                ...
                                                creeps, Game.flags[remote_flag].room.name, flag_structures
                                                , flag_constructions, False,
                                                True)
                                                      }, ).path
                                # 길 찾은 후 도로건설
                                for pos in constr_roads_pos:
                                    # 방 밖까지 확인할 필요는 없음......해볼까?
                                    # if pos.roomName != constr_pos.roomName:
                                    #     break
                                    pos.createConstructionSite(STRUCTURE_ROAD)

                        # 겹친게 아닐 경우. 기존 했던것처럼 한다.
                            ...
                            if not containers_around:
                                ...
                                print('Game.flags[{}].room.name: {}'.format(remote_flag, Game.flags[remote_flag].room.name))
                        ...
                        if Game.getObjectById(carrier_pickup):
                            ...
                            # 만일 소스가 합쳐져 있으면 길이 두배
                            distance = len(path) * merged_container
                            ...
                            spawning = spawn.createCreep(body, undefined,
                                                         {'role': 'carrier',
                                                          ...
                                                          'rm_flag': remote_flag, 'pickup': carrier_pickup
                                                             , 'work': working_part,
                            ...
                            if spawning == 0:
                                ...
                                spawning = spawn.createCreep(
                                    body,
                                    undefined,
                                    {'role': 'carrier', 'assigned_room': Game.flags[remote_flag].room.name,
                                     'rm_flag': remote_flag, 'pickup': carrier_pickup, 'work': working_part
                        ...
                        # 픽업이 존재하지 않는다는건 현재 해당 건물이 없다는 뜻이므로 새로 지어야 함.
                            ...
                            spawning = spawn.createCreep(
                                ...
                                undefined,
                                {'role': 'carrier', 'assigned_room': Game.flags[remote_flag].room.name,
                                 'rm_flag': remote_flag, 'work': True, 'home_room': spawn.room.name
                                    , 'source_num': carrier_source})
                    ...
                    # ************************ 캐리어 수정 끝 - 180313
                        ...
                        regular_spawn = spawn.createCreep(
                            [WORK, WORK, WORK, WORK, WORK, WORK,
                             ...
                             MOVE, MOVE]
                            , undefined,
                            {'role': 'harvester', 'assigned_room': Game.flags[remote_flag].room.name,
                             'home_room': spawn.room.name,
                             'size': 2, 'rm_flag': remote_flag})
                        # print('what happened:', regular_spawn)
                        if regular_spawn == -6:
                            spawn.createCreep([WORK, WORK, WORK, WORK, WORK,
                                              ...
                                              , undefined,
                                              {'role': 'harvester', 'assigned_room': Game.flags[remote_flag].room.name
                                                  , 'home_room': spawn.room.name, 'rm_flag': remote_flag})
                            continue
                    ...
                    if Game.cpu.bucket > cpu_bucket_emergency + cpu_bucket_emergency_spawn_start:
                        ...
                        for fn in Object.keys(flags):
                            ...
                            if not Game.flags[fn].room:
                                ...
                                if Game.flags[remote_flag].room.name == Game.flags[fn].room.name \
